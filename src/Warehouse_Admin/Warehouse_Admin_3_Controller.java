...
/**
 * Warehouse admin_3 controller class
 * 
 * @author Jatin
 *
 */
public class Warehouse_Admin_3_Controller {
	// private Stage primaryStage;

	public Warehouse_Admin_3_Controller() {
	...
	void next_window(ActionEvent event) throws IOException {
		...
		if (!name.getText().isEmpty() && !path.getText().isEmpty()) {
			...
			if (w.getInventory().CheckInProducts(name.getText()) == null) {
				try {
					w.getInventory().DeleteCategory(Path);
				...
				try {
					w.getInventory().DeleteProduct(name.getText(), VPath);
