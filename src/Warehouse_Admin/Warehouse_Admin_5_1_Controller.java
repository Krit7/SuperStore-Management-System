...
/**
 * Warehouse admin_5_1 controller class
 * @author 
 *
 */
public class Warehouse_Admin_5_1_Controller implements Initializable {
	public Category c;
	...
	void next_window(ActionEvent event) {
		...
		if (!List.getItems().isEmpty()) {
			String filename;
			c = SuperUser.Warehouses.get(Integer.parseInt(prevStage.getTitle().split(" ")[1])).getAdmin().c[0];

			c = SuperUser.Warehouses.get(Integer.parseInt(prevStage.getTitle().split(" ")[1])).getAdmin().c[1]
			...
			if (c.CheckForProductOrCategory() == 0) {
				...
				SuperUser.Warehouses.get(Integer.parseInt(primaryStage.getTitle().split(" ")[1])).getAdmin().flag = 1;
	...
	private void load_warehouse_list() {
		c = SuperUser.Warehouses.get(Integer.parseInt(primaryStage.getTitle().split(" ")[1])).getAdmin().c[0];
		if (SuperUser.Warehouses.get(Integer.parseInt(primaryStage.getTitle().split(" ")[1])).getAdmin().flag == 0) {
			c = SuperUser.Warehouses.get(Integer.parseInt(primaryStage.getTitle().split(" ")[1])).getAdmin().c[0];
		} else {
			c = SuperUser.Warehouses.get(Integer.parseInt(primaryStage.getTitle().split(" ")[1])).getAdmin().c[1];
